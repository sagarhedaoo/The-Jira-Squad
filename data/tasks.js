...
async function getTask(taskId) {
  ...
  for (let i = 0; i < task.subtasks.length; i++) {
    ...
    for (let j = 0; j < task.subtasks[i].comments.length; j++) {
      task.subtasks[i].comments[j]._id =
        task.subtasks[i].comments[j]._id.toString();
    }
  }
...
async function editTask(taskId, title, description, dueDate) {
  ...
  const updateInfo = await projectCollection.updateOne(
    ...
    {
      $set: {
        "tasks.$[updateTask].title": title,
        "tasks.$[updateTask].description": description,
        "tasks.$[updateTask].dueDate": dueDate,
      },
    },
...
async function moveTask(taskId, forward) {
  ...
  const newStage = forward
    ? Math.min(task.stage + 1, 3)
    : Math.max(task.stage - 1, 0);
...
async function assignTask(taskId, userId) {
  ...
  let task = await getTask(taskId);
  const user = await projectData.findUserInProject(
    userId,
